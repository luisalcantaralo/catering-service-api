...
router.put('/:id', async(req, res, next) => {
    ...
    client.connect();
    const {id} = req.params;
    const { order_event, order_notes, recurring,  people, products } = req.body;
    ...
    try {
        ...
        for (let i=0; i<products.length; ++i){
          ...
          total_price += parseFloat(price) * parseFloat(products[i]["quantity"]);
          queryParameters.push([id, products[i]["id"], products[i]["quantity"], parseFloat(price) * parseFloat(products[i]["quantity"])]);
        }

        let data = await client.query('UPDATE orders SET order_event = $3, recurring = $4, order_notes= $5, amt_people= $6, total_price= $2 WHERE order_id = $1', [id, total_price, order_event, recurring, order_notes, people]);

        await client.query('DELETE FROM order_product WHERE order_id = $1', [id]);
