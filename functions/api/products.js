...
router.put('/:id', async(req, res, next) => {
    ...
    const {product_name, description, category, price, measure} = req.body;
    try {
        const data = await client.query('UPDATE products SET product_name = $2, description = $3, category = $4, price = $5, measure = $6 WHERE customer_id = $1', [id, product_name, description, category, price, measure]);
        console.log(data.rows) ;
